  ...
  async execute(sock, m) {
    try {
      if (m.quoted && m.quoted.text) {
        ...
        v.reply(m.chat, 'Por favor, proporciona un enlace de TikTok para descargar el video.', { quoted: m });
      }
    } catch (error) {
...
const tiktokDownloader = async (sock, m, tiktokUrl) => {
  try {
    if (lastDownloads.has(m.chat)) {
      v.reply(m.chat, 'Espera un momento antes de realizar otra descarga.', { quoted: m });
      return;
    }
    ...
    if (response && response.result && response.result.video && response.result.video.noWatermark) {
      ...
      v.reply(m.chat, 'Error al obtener información del video de TikTok.', { quoted: m });
    }
  } catch (error) {
    console.error('Error en la función tiktokDownloader:', error);
    v.reply(m.chat, 'Error al procesar la solicitud. Intenta de nuevo más tarde.', { quoted: m });
  }
};
