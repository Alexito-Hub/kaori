...
const dbManager = {
  ...
  boolean: (propertyPath) => {
    const db = getDatabase();
    const [objectKey, propertyKey] = propertyPath.split('.');
    const object = db[objectKey];

    if (object && typeof object[propertyKey] === 'boolean') {
      object[propertyKey] = !object[propertyKey];
      updateDatabase(db);
      console.log(`Cambiado ${propertyPath} a ${!object[propertyKey]}`);
    } else {
